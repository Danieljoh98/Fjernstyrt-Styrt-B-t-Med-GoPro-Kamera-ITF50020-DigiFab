...
# valg av pins som skal bli brukt
...
"""
# lage vente tid for å koble seg opp på wifi
max_wait = 10
while max_wait > 0:
...
# sjeker om den får tilgang eller ikke. gir ut ip som webserveren er på 
if wlan.status() != 3:
...
# sjeker og styrer venstre motor
        if left_motor_on != -1:
            print("left_motor on")
            left_motor.value(1)
...
# sjeker og styrer begge motorene samtidig (rett fram)
        if both_motors_on != -1:
